...
REM Upgrade pip
echo Fixing pip + setuptools compatibility...
python -m pip install --upgrade pip wheel
python -m pip uninstall -y setuptools
python -m pip install --no-cache-dir "setuptools<71"
...
echo Installing git packages (no build isolation)...
pip install --no-build-isolation git+https://github.com/mgb45/pretrained-backbones-unet.git
pip install --no-build-isolation git+https://github.com/mgb45/CLIP.git
pip install --no-build-isolation git+https://github.com/mgb45/pytorch_diffusion.git
if %ERRORLEVEL% NEQ 0 (
    echo ERROR: Failed to install git packages
    pause
    exit /b 1
)
